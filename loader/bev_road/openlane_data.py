...
class OpenLane_dataset_with_offset(Dataset):
    def __init__(self, image_paths, 
        ...
        for card in card_list:
            ...
            start_idx += len(gt_list)
            self.dir_len.append(start_idx)
            for cnt in gt_list:
    ...
    def get_seg_offset(self, idx1, smooth=False):
        if True: # multi frame setting
            time_step = random.randint(1, 5)
            all_start_idx = np.array(self.dir_len[:-1])
            curr_start_idx = all_start_idx[np.max(np.where(idx1 >= all_start_idx))]
            if idx1 - curr_start_idx >= time_step:
                temporal_image_idxs = [idx1-time_step, idx1]
            else:
                temporal_image_idxs = [curr_start_idx, idx1]
        else:
            temporal_image_idxs = [idx1]
        ...
        for temporal_idx in temporal_image_idxs:
            gt_path = os.path.join(self.gt_paths, self.cnt_list[temporal_idx][0], self.cnt_list[temporal_idx][1])
            image_path = os.path.join(self.image_paths, self.cnt_list[temporal_idx][0], self.cnt_list[temporal_idx][1].replace('json', 'jpg'))
            image = cv2.imread(image_path)
            image_h, image_w, _ = image.shape
            with open(gt_path, 'r') as f:
                gt_time = json.load(f)
            cam_extrinsics_l.append(np.array(gt_time['extrinsic'])@ maxtrix_camera2camera_w)
            cam_intrinsic_l.append(np.array(gt_time['intrinsic']))

            images.append(image)
            gt_paths.append(gt_path)

        with open(gt_paths[-1], 'r') as f:
            gt = json.load(f)
        ...
        assert (cam_extrinsics == cam_extrinsics_l[-1]).all() and (cam_intrinsic == cam_intrinsic_l[-1]).all()
        R_vg = np.array([[0, 1, 0],
                         [-1, 0, 0],
                         [0, 0, 1]], dtype=float)
        R_gc = np.array([[1, 0, 0],
        ...
        if self.use_virtual_camera:
            for i, image in enumerate(images):
                sc = Standard_camera(self.vc_intrinsic, self.vc_extrinsics, self.vc_image_shape,
                                     cam_intrinsic_l[i], cam_extrinsics_l[i], (image.shape[0], image.shape[1]))
                ...
                vr_images.append(image)

            return vr_images, image_gt, ipm_gt, offset_y_map, z_map, cam_extrinsics, cam_intrinsic # all np
        return images, image_gt, ipm_gt, offset_y_map, z_map, cam_extrinsics, cam_intrinsic
    ...
    def __getitem__(self, idx):
        ...
        # img = images[-1,].permute(1, 2, 0).detach().cpu().numpy()
        # cv2.imwrite(f'/media/img/imgs_last_unit/last_input_{idx}.jpg', img)
        ''' 2d gt'''
        ...
        return images, ipm_gt_segment.float(), ipm_gt_instance.float(), ipm_gt_offset.float(), ipm_gt_z.float(), image_gt_segment.float(), image_gt_instance.float()
...
class OpenLane_dataset_with_offset_val(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if True:  # multi frame
            time_step = 5
            all_start_idx = np.array(self.dir_len[:-1])
            curr_start_idx = all_start_idx[np.max(np.where(idx >= all_start_idx))]
            if idx - curr_start_idx >= time_step:
                temporal_image_idxs = [idx - time_step, idx]
            else:
                temporal_image_idxs = [curr_start_idx, idx]
        else:
            temporal_image_idxs = [idx]
        imgs = []
        for idx in temporal_image_idxs:
        ...
        return images, self.cnt_list[temporal_image_idxs[-1]]
