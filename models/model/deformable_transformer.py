# ------------------------------------------------------------------------
# CaliFree3DLane Transformer classes.
# ------------------------------------------------------------------------
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 num_frames=2, num_feature_levels=4,
        ...
        self.time_embed = nn.Parameter(torch.Tensor(num_frames, d_model))
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        bs, nf,  _, c = memory.shape # 2 1 720 512
       
            
        query_embed, tgt = torch.split(query_embed, c, dim=1)
        query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1) # query pos
        tgt = tgt.unsqueeze(0).expand(bs, -1, -1)
        reference_points = self.reference_points(query_embed).sigmoid() # directly
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        self.linear1_off = nn.Linear(256, d_ffn // 2)
        self.activation_off = _get_activation_fn(activation)
        self.linear2_off = nn.Linear(d_ffn // 2, d_ffn // 2)
        self.linear3_off = nn.Linear(d_ffn // 2, 2)

        self.linear1_map = nn.Linear(2, 128)
        self.activation_map = _get_activation_fn(activation)
        self.linear2_map = nn.Linear(128, 128)
        self.linear3_map = nn.Linear(128, 128)
    ...
    def reference_points_mapping(self, tgt):
        tgt2 = self.activation_map(self.linear2_map(self.activation_map(self.linear1_map(tgt))))
        tgt = self.linear3_map(tgt2)
        return tgt

    def reference_points_bias(self, tgt):
        tgt2 = self.activation_off(self.linear2_off(self.activation_off(self.linear1_off(tgt))))
        tgt = self.linear3_off(tgt2)
        return tgt

    def forward(self, tgt, tgt_box, query_pos, reference_points, reference_points_input, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        # self attention
        bs, nf, n_query, _, _ = reference_points_input.shape  # 6 1 125 2 2
        ...
        if len(tgt_box.shape) == 3:  # tgt_box [bz,300,C]. # first layer
            ...
            tgt2_box = self.self_attn_box(q_box.transpose(0, 1), k_box.transpose(0, 1), tgt_box.transpose(0, 1))[
                0].transpose(0, 1)
            ...
            N, nf, _, _ = src.shape
            _, _, num_q, C = tgt_box.shape
            tgt_list = []
            for i_f in range(tgt_box.shape[1]): #
                tgt_box_i = tgt_box[:, i_f]
                q_box = k_box = self.with_pos_embed(tgt_box_i, query_pos)
                tgt2_box_i = \
                    self.self_attn_box(q_box.transpose(0, 1), k_box.transpose(0, 1), tgt_box_i.transpose(0, 1))[
                        0].transpose(0, 1)
                tgt_box_i = tgt_box_i + self.dropout2_box(tgt2_box_i)
                tgt_box_i = self.norm2_box(tgt_box_i)
                tgt_list.append(tgt_box_i.unsqueeze(1))
            tgt_box = torch.cat(tgt_list, dim=1) # 1 2125 512
            if tgt_box.shape[1] != nf:
                tgt_box = tgt_box.repeat(1, nf, 1, 1) # 1 2 125 512

            # cross attention
            tgt2, tgt2_box, sampling_locations, attention_weights = self.cross_attn(self.with_pos_embed(tgt, query_pos),
                                                                                    self.with_pos_embed_multf(tgt_box,
                                                                                                              query_pos),
                                                                                    reference_points_input, src,
                                                                                    src_spatial_shapes,
                                                                                    level_start_index, src_padding_mask)
        ...
        tgt2 = (tgt2 * time_weight).sum(1)  # tgt2是instance query
        ...
        if not first_layer:
            sampling_locations = sampling_locations.view(bs, nf, n_query, -1)
            reference_points = self.reference_points_mapping(reference_points.view(bs, nf, n_query, -1))
            reference_points = torch.cat((reference_points, sampling_locations), -1)
            reference_points = self.reference_points_bias(reference_points).sigmoid()  # 1 2 125 2
            # import pdb; pdb.set_trace()
        return tgt, tgt_box, sampling_locations, attention_weights,reference_points
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output, output_box, sampling_locations, attention_weights, reference_points = \
                 layer(output, output_box, query_pos, reference_points, reference_points_input,
        ...
        return output, reference_points, None, None
