...
def train_epoch(model, dataset, optimizer, configs, epoch):
    ...
    input_data, gt_seg_data, gt_emb_data, offset_y_data, z_data, image_gt_segment, image_gt_instance) in enumerate(
        ...
        # img = input_data[1, 1, ].permute(1, 2, 0).detach().cpu().numpy()
        # cv2.imwrite(f'test_{idx}.jpg', img)
        bs, n, c, h, w = input_data.size()
        input_data = input_data.view(bs*n, c, h, w).contiguous()
...
def worker_function(config_file, gpu_id, checkpoint_path=None):
    ...
    print(configs.loader_args)
    print(configs.vc_config['use_virtual_camera'])
    print(model)
...
if __name__ == '__main__':
    ...
    worker_function('./openlane_config.py', gpu_id=[0, 1, 2, 4])