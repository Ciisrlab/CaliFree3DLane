...
import time
import sys
sys.path.append('/media/sda2/CaliFree3DLane')
import os
gpu_id = [1]
...
# model_path = '/media/sda2/CaliFree3DLane/dataset/model/OpenLane/latest.pth' #model path of verification

tmp_save_path = '/media/sda2/CaliFree3DLane/checkpoint/openlane_time_tmp' #tmp path for save intermediate result
...
def val():
    print_model = True
    dir = '/media/sda2/CaliFree3DLane/checkpoint/openlane_time'
    ...
    for ep in dir_list:
        ...
        np_save_path = os.path.join(tmp_save_path, 'openlane' + '_np')
        os.makedirs(np_save_path, exist_ok=True)
        # res_save_path = os.path.join(tmp_save_path, str(time1) + '_result')
        res_save_path = os.path.join(tmp_save_path, 'openlane' + '_result')
        ...
        for item in tqdm(val_loader):
            ...
            image = image.view(bs * n, c, h, w).contiguous()
            with torch.no_grad():
                pred_ = model(image)[0]
